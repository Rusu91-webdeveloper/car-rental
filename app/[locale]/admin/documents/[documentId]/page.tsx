...
async function loadReviewPage(documentId: string) {
  ...
  const replacementHistory = await context.repository.listSessionDocuments(
    context.scope.uploadSessionId ?? "",
  )
  return {
    document,
...
export default async function DocumentReviewPage({
  ...
  let state:
    | Awaited<ReturnType<typeof loadReviewPage>>
    | { reauthenticate: true }
    | { error: string }
  try {
    state = await loadReviewPage(documentId)
  } catch (error) {
    if (error instanceof PrivateDocumentError && error.code.startsWith("RECENT_AUTH_"))
  ...
  if ("reauthenticate" in state)
    return <main className="mx-auto max-w-2xl p-6"><ReauthenticatePanel returnTo={returnTo} /></main>
  if ("error" in state)
    return <main className="mx-auto max-w-2xl p-6"><h1 className="text-2xl font-semibold">Review unavailable</h1><p className="mt-3 text-muted-foreground">{state.error}</p></main>
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6">
      <header><p className="text-sm font-medium text-primary">Restricted document operations</p><h1 className="text-2xl font-semibold">Secure document review</h1></header>
