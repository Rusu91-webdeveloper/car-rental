...
export default async function BillingConfigurationPage() {
  ...
  return <div className="space-y-6"><PricingVersionHeader data={data} title="Billing rules" description="Choose how rental duration becomes billable days and how daily, weekly, and fixed-month prices are combined. Calendar-month pricing is intentionally unavailable." /><PricingDraftControls data={data} canManage={capabilities.canManagePricing} canValidate={capabilities.canValidate} /><BillingRuleForm key={`${data.draftPricing?.id ?? "none"}-${data.draftPricing?.revision ?? 0}`} data={data} canManage={capabilities.canManagePricing} /><QuotePreviewPanel data={data} /><DraftLivePricingComparison data={data} /><PricingIssueList issues={data.issues} title="Billing and strategy checks" /></div>
}
