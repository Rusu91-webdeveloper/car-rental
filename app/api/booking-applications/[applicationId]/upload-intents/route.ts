...
export async function POST(
  ...
  try {
    ...
    await enforceRateLimit(
      "upload:intent",
      context.user.id,
      PHASE8FB_RATE_LIMITS.uploadIntent,
    );
    ...
    if (rateLimitSubject && !(error instanceof RateLimitExceededError)) {
      try {
        await enforceRateLimit(
          "upload:invalid",
          rateLimitSubject,
          PHASE8FB_RATE_LIMITS.invalidUpload,
        );
    ...
    const code =
      error instanceof PrivateDocumentError
        ...
        : error instanceof RateLimitExceededError
          ? "RATE_LIMITED"
          : "DOCUMENT_UPLOAD_REQUEST_INVALID";
    const retryAfter =
      error instanceof RateLimitExceededError
        ? String(error.retryAfterSeconds)
        : undefined;
    console.warn("[private-documents] upload intent rejected", {
      applicationId: applicationIdForLog,
      code,
      retryable:
        error instanceof PrivateDocumentError ? error.retryable : false,
    });
    return Response.json(
      { code },
      {
        status: code === "RATE_LIMITED" ? 429 : 409,
        headers: {
          "Cache-Control": "private, no-store",
