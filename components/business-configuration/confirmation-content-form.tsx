...
export function ConfirmationContentForm({ data, canEdit }: { data: PageData; canEdit: boolean }) {
  ...
  const summary = draft?.changeSummary ?? "Booking confirmation update"
  ...
  return (
    <section className="rounded-xl border bg-background p-5">
      ...
      <Button
        className="mt-4"
        disabled={!draft || !canEdit || pending}
        onClick={() =>
          startTransition(async () => {
            if (!draft) return
            const result = await updateConfirmationContentDraftAction({
              versionId: draft.id,
              ...
              configuration: {
                ...
                content: content.map((item) => ({
                  ...
                  safeContent: item.safeContent || undefined,
                })),
              },
            })
