...
export function ConfigurationIssueList({ title, issues, emptyMessage }: { title: string; issues: readonly ConfigurationHealthFinding[]; emptyMessage?: string }) {
  return (
        ...
        <ul className="mt-4 space-y-3">
          {issues.map((issue) => (
            <li key={`${issue.domain}-${issue.code}-${issue.affectedResource ?? "general"}`} className="rounded-lg border p-3">
              ...
              {issue.suggestedAction ? <p className="mt-2 text-sm text-muted-foreground">What to do: {issue.suggestedAction}</p> : null}
              {issue.adminRoute ? (
