...
describe("owner-facing admin information architecture", () => {
  ...
  it("saves setup progress and advances after successful form submissions", () => {
    ...
    expect(helper).toContain('`/admin/settings?saved=${stepId}`')
    expect(settings).toContain("const savedStep")
  ...
  it("uses section-specific writes against the existing CompanySettings source", () => {
    ...
    expect(actions).toContain("tx.companySettings.upsert")
    expect(actions).toContain('recordSettingsAudit(tx, admin.id, existing, validated, "business_profile_updated")')
    expect(actions).toContain('recordSettingsAudit(tx, admin.id, existing, validated, "payment_details_updated")')
    expect(actions).toContain('recordSettingsAudit(tx, admin.id, existing, validated, "notification_contacts_updated")')
